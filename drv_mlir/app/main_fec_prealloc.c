...
// 外部函数：MLIR 模型推理
extern void process_lidar(float *input_payload, float *drm_framebuffer, float* workspace);
...
int main(int argc, char **argv) {
    ...
    uint32_t workspace_size = 2 * 1024 * 1024; // 2MB
    float *workspace_ptr = (float *)malloc(workspace_size);
    if (!workspace_ptr) {
        perror("Workspace malloc failed");
        goto cleanup;
    }
                ...
                if (eth_frame[14 + 9] == 0x11) {
                    ...
                    if (dest_port == 8888) {
                        ...
                        if (payload_len >= 256) {
                            ...
                            process_lidar(tensor_in, drm_ptr, workspace_ptr);
