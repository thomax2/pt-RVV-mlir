...
// ====================== CPU 频率控制相关 ======================
static char original_governor[64] = {0};
static int low_freq = 0, high_freq = 0;

// 读取 scaling_available_frequencies，获取最低和最高频率（假设升序排列）
static void get_min_max_freq(int *min, int *max) {
    FILE *fp = fopen("/sys/devices/system/cpu/cpu0/cpufreq/scaling_available_frequencies", "r");
    if (!fp) {
        // 默认值（i.MX6ULL 典型 OPP）
        *min = 198000;
        *max = 900000;
...
// 保存当前的 scaling_governor，用于退出时恢复
static void save_original_governor(void) {
    ...
    if (fp) {
        if (fgets(original_governor, sizeof(original_governor), fp)) {
            // 去掉末尾换行符
            char *nl = strchr(original_governor, '\n');
            if (nl) *nl = '\0';
        }
        fclose(fp);
    } else {
        strcpy(original_governor, "ondemand"); // 默认
    }
}

// 恢复原始的 scaling_governor
static void restore_original_governor(void) {
    if (original_governor[0]) {
...
// 设置 governor 为 userspace 并强制设定频率
static void set_freq_userspace(int target_freq) {
    FILE *fp;
    ...
    if (fp) {
        ...
        perror("set governor userspace failed");
        return;
    }
    ...
    if (fp) {
        fprintf(fp, "%d", target_freq);
        fclose(fp);
    } else {
        perror("set scaling_setspeed failed");
    }
}

// 设置 governor 为 ondemand（让内核自动调频）
static void set_governor_ondemand(void) {
    FILE *fp = fopen("/sys/devices/system/cpu/cpu0/cpufreq/scaling_governor", "w");
    if (fp) {
        fprintf(fp, "ondemand");
        fclose(fp);
    } else {
        perror("set governor ondemand failed");
    }
}
// ==============================================================
...
void* power_manager_thread(void* arg) {
    ...
    printf(">> [看门狗] 电源管理线程已启动！(5秒超时机制，含 CPU 调频)\n");
    int last_screen_state = screen_is_on; // 记录上一次状态，避免重复操作

    while (keep_running) {
        long current_time = time(NULL);
        long idle_time = current_time - atomic_load(&last_rx_timestamp);

        // 逻辑 1: 屏幕亮着，且闲置超过 5 秒 -> 关屏 + 降频
        if (screen_is_on && idle_time >= 5) {
            printf("\n[看门狗] 5 秒无数据，执行息屏 + CPU 降频\n");
            if (args->use_atomic) {
            ...
            set_freq_userspace(low_freq);   // 强制最低频率
            screen_is_on = 0;
        }
        // 逻辑 2: 屏幕关着，但数据恢复了 (idle_time < 5) -> 亮屏 + 恢复性能
        else if (!screen_is_on && idle_time < 5) {
            printf("\n[看门狗] 数据恢复，唤醒屏幕 + 恢复 CPU 性能\n");
            if (args->use_atomic) {
                drm_atomic_set_power(args->drm_fd, args->crtc_id, args->conn_id,
            ...
            set_governor_ondemand();        // 恢复 ondemand 动态调频
            screen_is_on = 1;
        }
        sleep(1);
    }
...
int main(int argc, char **argv) {
    ...
    // ========== CPU 频率初始化 ==========
    get_min_max_freq(&low_freq, &high_freq);
    save_original_governor();
    printf(">> CPU 频率范围: 最低 %d kHz, 最高 %d kHz\n", low_freq, high_freq);
    // 初始状态：屏幕亮，使用 ondemand 动态调频
    set_governor_ondemand();
    // ===================================
    ...
    if (drm_fd < 0 || drm_ptr == NULL) {
        ...
        if (!drm_ptr) {
            ...
            goto cleanup; // 注意：goto 前没有创建线程等资源，但 cleanup 会处理
        }
    } else {
        // 检查驱动是否支持 atomic
...
cleanup:
    ...
    // 等待看门狗线程退出
    if (pm_tid) pthread_join(pm_tid, NULL);
