...
fec_enet_rx_queue(struct net_device *ndev, int budget, u16 queue_id)
{
	...
	while (!((status = fec16_to_cpu(bdp->cbd_sc)) & BD_ENET_RX_EMPTY)) {
        ...
        /* ==================== 新增代码开始 ==================== */
        /* 零拷贝拦截：如果在开启状态下，直接处理并跳过网络栈 */
        if (fep->zc_enabled) {
            /* 1. 同步 Cache，让 CPU 能读取 DMA 刚写入的数据 */
            dma_sync_single_for_cpu(&fep->pdev->dev,
                                    fec32_to_cpu(bdp->cbd_bufaddr),
                                    FEC_ENET_RX_FRSIZE,
                                    DMA_FROM_DEVICE);
            ...
            /* 2. 这里是你的处理逻辑 */
            /* 比如：wake_up_interruptible(&fep->zc_waitqueue); */
            /* 注意：此时数据在 phys_to_virt(fec32_to_cpu(bdp->cbd_bufaddr))， */
            /* 或者你可以通过 rxq->rx_skbuff[index] 找到对应的 buffer 地址 */

			atomic_inc(&fep->zc_pkt_count);
            wake_up_interruptible(&fep->zc_waitqueue);
...
rx_processing_done:
		/* Clear the status flags for this buffer */
		status = (status & BD_SC_WRAP) | BD_ENET_RX_EMPTY;
...
static int fec_enet_ioctl(struct net_device *ndev, struct ifreq *rq, int cmd)
{
	...
	/* ==================== 修正：移到最前面 ==================== */
	/* 必须在 netif_running 检查之前处理，允许在网卡 down 时配置 */
	if (cmd == FEC_IOC_SETUP_ZC) {
		if (copy_from_user(&zc_cfg, rq->ifr_data, sizeof(zc_cfg)))
			return -EFAULT;

		fep->zc_enabled = zc_cfg.enable;
		if (fep->zc_enabled) {
			if (zc_cfg.dma_addr % 64 != 0) {
				netdev_err(ndev, "FEC ZC: DMA address must be 64-byte aligned!\n");
				return -EINVAL;
			}
			fep->zc_dma_addr = (dma_addr_t)zc_cfg.dma_addr;
			fep->zc_size = zc_cfg.size;

			/* 初始化等待队列和包计数器 */
            init_waitqueue_head(&fep->zc_waitqueue);
            atomic_set(&fep->zc_pkt_count, 0);

			netdev_info(ndev, "FEC Zero-Copy enabled, DMA Addr: 0x%08x, Size: %u\n", 
						(unsigned int)fep->zc_dma_addr, zc_cfg.size);
		} else {
			netdev_info(ndev, "FEC Zero-Copy disabled\n");
		}
		return 0; /* 配置成功，直接返回 */
	}
	/* ======================================================= */

	if (cmd == FEC_IOC_WAIT_ZC_DATA) {
        if (!fep->zc_enabled) return -EINVAL;

        /* * 让当前进程进入休眠 (CPU 占用降为 0%)
         * 直到 zc_pkt_count 大于 0，或者被 Ctrl+C (信号) 打断
         */
        if (wait_event_interruptible(fep->zc_waitqueue, atomic_read(&fep->zc_pkt_count) > 0)) {
            return -ERESTARTSYS; // 如果是被 Ctrl+C 打断的，返回这个错误码
        }

        /* 被唤醒了！说明有数据了，消费掉一个计数，返回用户态 */
        atomic_dec(&fep->zc_pkt_count);
        return 0;
    }

	/* 原有逻辑：如果网卡 down 了，拒绝后续的 PHY 和时间戳配置 */
	if (!netif_running(ndev))
		return -EINVAL;
...
fec_enet_alloc_rxq_buffers(struct net_device *ndev, unsigned int queue)
{
	...
	/* 新增：计算 ZC 模式下每一片的物理地址 */
	dma_addr_t current_zc_dma = fep->zc_dma_addr; 
	...
	for (i = 0; i < rxq->bd.ring_size; i++) {
		...
		/* ==================== 新增：零拷贝模式处理 ==================== */
		if (fep->zc_enabled) {
			// 1. 确保 dma-buf 总大小足够分给整个 Ring
			if (fep->zc_size < (rxq->bd.ring_size * FEC_ENET_RX_FRSIZE)) {
				netdev_err(ndev, "FEC ZC: dma-buf size too small for RX ring!\n");
				goto err_alloc;
			}
			
			// 2. 不分配 skb，将其置空，防止后续代码误操作
			rxq->rx_skbuff[i] = NULL;
			
			// 3. 直接将你的 dma-buf 切片物理地址填给硬件
			bdp->cbd_bufaddr = cpu_to_fec32(current_zc_dma);
			current_zc_dma += FEC_ENET_RX_FRSIZE; // 偏移 2048 字节给下一个描述符
